# PersonAIs — Backend Engine (Alpha 0.4.2)
...
## What was broken in 0.4.1, and what 0.4.2 changes
...
| `ANTHROPIC_WORKSPACE_ID` | no       | unset                         | Only for a key that is not already scoped to a workspace. |
...
3. **`"engine_selftest": "ok"`** with `engine_selftest_detail` reading
   ...
   reply. Chat works.
4. **`"model_status"`** — `served` is the good case. `corrected` means the id
...
5. **`"models"` must show the same id for `free` and `pro`.** If `pro` reads
   `claude-fable-5`, the old build is still live.
6. **Send one chat message as a pro/ultra account** — the case that was
...
- There is no pro-tier model. Every paying tier is served by the same model as
  the free tier until one is available on this API key.
